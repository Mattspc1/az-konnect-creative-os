...
function rHTMLAds(el) {
    const subs = [`${C.product}. ${C.metric}. ${C.result}. ${C.urgency}.`, `${C.solution} for ${C.audience.toLowerCase()}. Results in days.`, `${C.result} \u2014 on complete autopilot. ${C.price}.`, `Stop ${C.old.toLowerCase()}. Start winning. ${C.urgency}.`, `Built for ${C.audience.toLowerCase()} who want real results.`, `${C.metric} automated. ${C.result}. Zero manual work.`, `The #1 ${C.solution} in ${C.niche}. ${C.price}.`, `Replace ${C.old.toLowerCase()} with ${C.solution}. Live in ${C.urgency}.`, `${C.solution} by ${C.company}. ${C.price}. Money-back guarantee.`, `${C.product}: ${C.metric}. No contracts. No risk.`, `Trusted by top ${C.audience.toLowerCase()} since 2020.`, `${C.result}. Week one. On autopilot. ${C.price}.`, `${C.solution} that works. ${C.price}. ${C.urgency}.`, `${C.metric}. ${C.result}. Money-back guarantee.`, `Scale faster with ${C.product}. ${C.urgency}.`, `Outperform the competition. ${C.metric}.`, `${C.solution} by ${C.company}. Live in ${C.urgency}.`, `From zero to ${C.result} in one week.`, `${C.product}: the ${C.solution} for ${C.niche}.`, `${C.audience} are switching to ${C.solution}.`, `${C.metric}. ${C.result}. Start now.`, `Why top performers chose ${C.product}.`, `The smart way to grow. ${C.urgency}. ${C.price}.`, `One week. ${C.result}. Guaranteed.`];
...
function genIGPosts(el) {
    const posts = [
        ...
        { h: `Your competitor just hit ${C.result}.`, b: `They didn't grind harder. They switched to ${C.solution}. ${C.product} gives you the same edge. ${C.price}. Money-back guarantee.`, v: 'Dark gradient background with urgency-style countdown or limited spots indicator', cta: 'Tap the link before spots fill' },
